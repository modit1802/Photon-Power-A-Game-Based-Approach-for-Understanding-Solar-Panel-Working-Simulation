...
class BandTransition:
    def __init__(self):
        self.electron_y = 395  # start at valence band
        self.hole_y = 395
        self.electron_x = 50
        self.active = True
        self.timer = 60
    ...
    def update(self):
        if self.timer > 0:
            self.electron_y -= 2  # moving up to conduction band
            self.timer -= 1
        else:
            self.active = False
    ...
    def draw(self):
        if self.active:
            pygame.draw.circle(screen, ORANGE, (self.electron_x, int(self.electron_y)), 6)
            pygame.draw.circle(screen, YELLOW, (self.electron_x, int(self.hole_y)), 6)
            pygame.draw.line(screen, RED, (self.electron_x, self.hole_y), (self.electron_x, self.electron_y), 2)
...
def main():
    ...
    # Intro screen
    screen.fill(BLACK)
    title_text = big_font.render("Gamified Simulation of the Photoelectric Effect in Semiconductors", True, WHITE)
    subtitle_text = font.render("By Modit(21803019), Aviral(21803021) & Swastik(21803017) | Batch B12", True, CYAN)
    ...
    while True:
        ...
        for bt in band_transitions[:]:
            ...
            bt.draw()
            if not bt.active:
                valence_electrons.append((bt.electron_x, 395))
        ...
        if not quiz_shown and time.time() - start_time >= 40:
            run_quiz()
            quiz_shown = True
